...
export function SearchPage({ session }: { session: VerifySession }) {
  ...
  const canSearch = useMemo(() => {
    // require at least two identifying fields
  ...
  async function submit() {
    ...
    setBusy(true);
    try {
      const r = await verifySearch(session.accessKey, payload);
      setResult(r);
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      <div style={styles.card}>
        ...
        <p style={styles.p}>Enter either <b>Badge token + PIN</b> or <b>Name + Badge last 4</b>.</p>
        ...
        <div style={styles.grid}>
          ...
          <div>
            ...
            <input style={styles.input} value={badgeToken} onChange={(e) => setBadgeToken(e.target.value)} placeholder="badge_xxx" />
          </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 14, flexWrap: 'wrap' }}>
          ...
          {!canSearch ? (
            <div style={styles.hint}>Requires: <b>Token + PIN</b> OR <b>Name + last 4</b></div>
          ) : null}
        </div>
      ...
      {result ? (
        <div style={styles.card}>
          ...
          <div style={{ marginTop: 8, opacity: 0.85 }}>
            Applicant: <b>{result.patron.first_name || ''} {result.patron.last_name || ''}</b>{' '}
            <span style={{ opacity: 0.8 }}>(Badge last4: {result.patron.badge_last4 || '—'})</span>
          </div>
              ...
              <tbody>
                {result.applications.map((a) => (
                  <tr key={a.id}>
                    ...
                    <td style={styles.td}><span style={styles.status}>{a.status}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
